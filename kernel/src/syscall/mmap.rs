...
pub(crate) fn sys_mmap(
    ...
    scheduler::with_current_task(|task| {
        let addr = if flags.contains(MemMapFlags::MAP_FIXED) {
            // alignment check
            if !addr.is_multiple_of(memory::PAGE_SIZE) {
                log::error!("MAP_FIXED address must be page-aligned");
                return -errno::EINVAL as _;
            ...
            // overlap check
            if task.vmspace.find(addr).is_some() {
            ...
            || !addr.is_multiple_of(memory::PAGE_SIZE)
            || task.vmspace.find(addr).is_some()
        {
        ...
        task.vmspace
            ...
            .expect("Failed to insert memory mapping");
